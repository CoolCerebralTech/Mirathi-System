...
import { 
  Injectable as SchedulerInjectable, 
  Logger as SchedulerLogger 
} from '@nestjs/common';
...
export class SchedulerService {
  ...
  constructor(
    private readonly auditingService: SchedulerAuditingService,
  ) {}
  ...
  async detectSuspiciousActivity(): Promise<void> {
    ...
    try {
      ...
      if (suspicious.length > 0) {
        this.logger.warn(
          `Found ${suspicious.length} users with suspicious activity`,
          suspicious
        );
      }
    } catch (error) {
