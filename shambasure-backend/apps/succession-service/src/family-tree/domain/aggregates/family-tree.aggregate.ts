...
export class FamilyAggregate extends AggregateRoot {
  ...
  removeFamilyMember(memberId: string): void {
    ...
    // This calls the method on the Entity (FamilyMember.ts)
    // If "Property does not exist" error occurs here, ensure FamilyMember.ts is updated.
    ...
    this.family.updateMemberStatus({
      memberId,
      becameDeceased: member.getIsDeceased(),
    });
  ...
  validateForSuccession(): { isValid: boolean; errors: string[]; warnings: string[] } {
    ...
    // FIX: Typed array to prevent Enum mismatch error
    ...
    const criticalRelationships = Array.from(this.relationships.values()).filter(
      (r) => criticalTypes.includes(r.getType()) && !r.getIsVerified(),
    );
