...
import { 
  JwtAuthGuard as WillJwtAuthGuard, 
  CurrentUser as WillCurrentUser, 
  JwtPayload as WillJwtPayload 
} from '@shamba/auth';
import { WillsService } from '../services/wills.service';
import { WillEntity, WillSummaryEntity, BeneficiaryAssignmentEntity } from '../entities/succession.entity';
...
@WillUseGuards(WillJwtAuthGuard)
...
export class WillsController {
  ...
  async create(
    @WillCurrentUser('sub') userId: string,
  ...
  async findMyWills(@WillCurrentUser('sub') userId: string): Promise<WillSummaryEntity[]> {
  ...
  async getActiveWill(@WillCurrentUser('sub') userId: string): Promise<WillEntity | null> {
  ...
  async findOne(
    ...
    @WillCurrentUser() user: WillJwtPayload
  ): Promise<WillEntity> {
