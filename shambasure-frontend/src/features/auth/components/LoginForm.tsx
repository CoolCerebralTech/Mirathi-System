  ...
  return (
    <div className="relative">
      ...
      <Input
        ...
        className={`pl-10 pr-10 ${error ? 'border-danger focus:border-danger focus:ring-danger/20' : 'border-neutral-300 focus:border-primary focus:ring-primary/20'} bg-background transition-all duration-300`}
        {...register}
      />
      <button
        type="button"
...
export function LoginForm() {
    ...
    login(
      {
        data: {
          ...formData,
          deviceId: deviceIdRef.current || undefined, // Attach the generated deviceId
        },
        rememberMe,
      },
      {
        onSuccess: () => {
          // Toast is now handled by the useLogin hook. This keeps concerns separated.
          // We only need to handle the navigation side-effect here.
          navigate(from, { replace: true });
        },
        onError: (error) => {
          // Error toast is also handled centrally by the useLogin hook.
          // We can log the specific component context if needed.
  ...
  // The rest of the JSX is beautifully designed and requires no changes.
  return (
    <div className="w-full space-y-8">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
        <div className="space-y-2">
          ...
          <div className="relative">
            ...
            <Input
              ...
              className={`pl-10 ${errors.email ? 'border-danger focus:border-danger focus:ring-danger/20' : 'border-neutral-300 focus:border-primary focus:ring-primary/20'} bg-background transition-all duration-300`}
              {...register('email')}
            />
          </div>
          {errors.email && (
        ...
        <div className="space-y-2">
          <div className="flex items-center justify-between">
            ...
            <Link to="/forgot-password" className="rounded text-sm font-medium text-primary transition-colors hover:text-primary-hover hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
        ...
        <div className="flex items-center space-x-2">
          ...
          />
          <Label htmlFor="remember" className="cursor-pointer select-none text-sm font-normal leading-none text-text-subtle peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
      ...
      <div className="space-y-4 text-center">
        <p className="text-sm text-text-subtle">{t('auth:no_account', 'Don\'t have an account yet?')}</p>
        <Link to="/register" className="inline-flex items-center gap-2 rounded font-serif text-base font-semibold text-primary transition-all duration-300 hover:text-primary-hover hover:underline focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2">
      ...
      <div className="rounded-elegant border border-secondary/20 bg-secondary/5 p-4">
        <p className="flex items-start gap-3 text-xs leading-relaxed text-text-subtle">
          ...
          <span>{t('auth:security_notice', 'Your connection is encrypted and secure. We use device tracking for enhanced security and comply with all KDPA requirements.')}</span>
        </p>
      </div>
    </div>
  );
