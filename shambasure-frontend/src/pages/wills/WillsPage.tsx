...
export function WillsPage() {
    ...
    deleteMutation.mutate(willToDelete.id, {
      ...
      onError: (error) => {
        ...
        toast.error(errorMessage, { 
          description: t('wills:delete_failed') 
        });
      },
    });
  };
  ...
  return (
    <div className="space-y-6">
      <PageHeader 
        title={t('wills:my_wills')} 
        description={t('wills:my_wills_subtitle')}
      >
      ...
      {/* Create/Edit Will Dialog */}
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            <DialogTitle>
              {willToEdit ? t('wills:edit_will') : t('wills:create_will')}
            </DialogTitle>
            <DialogDescription>
              {willToEdit 
      ...
      <AlertDialog open={!!willToDelete} onOpenChange={() => setWillToDelete(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {t('wills:confirm_delete_message', { title: willToDelete?.title || '' })}
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
